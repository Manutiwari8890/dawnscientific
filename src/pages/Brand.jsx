...
function Brand() {
    ...
    return (
        <>
            {brandDetail?.meta_title ?

                <Helmet>
                    <title>{brandDetail?.meta_title || brandDetail?.name} </title>
                    <meta
                        name="description"
                        content={brandDetail?.meta_description || `${brandDetail?.name} available now.`}
                    />
                    <meta
                        name="keywords"
                        content={brandDetail?.meta_keyword || brandDetail?.name.split(" ").join(", ")}
                    />
                </Helmet> :
                <Helmet>
                    <title>{`${brandDetail?.name} | Dawn Scientific`} </title>
                    <meta property="og:title" content={brandDetail?.name} />
                    <meta property="og:image" content={brandDetail?.image_url} />
                    <meta name="twitter:card" content="summary_large_image" />
                    <meta name="twitter:title" content={brandDetail?.name} />
                    <meta name="twitter:image" content={brandDetail?.image_url} />
                </Helmet>
            }
                ...
                <div className="container">
                    <div className="row">
                                        ...
                                        {categories.map(cat => (
                                            <li className={`${(cat.children_recursive.length > 0) ? 'has-children' : ''} ${((openToggle === cat.id)) ? 'active' : ''}`} key={cat.id}>
                                                <div className="list-text">
                                                    <NavLink to={`/product-category/${cat.slug}`} className={``} >
                                                        {cat.name}
                                                        ...
                                                        {cat.children_recursive.map((child) => (
                                                            (child.products_count > 0 &&
                                                                <li className={`${(child.children_recursive.length > 0) ? 'has-children' : ''} ${((openToggle === child.id)) ? 'active' : ''}`} key={child.id}>
                                                                    <div className="list-text">
                                                                        <NavLink to={`/product-category/${cat.slug}/${child.slug}`} className={''}>
                                                                            {child.name}
                                                                            ...
                                                                            {child.children_recursive.map((value) => (
                                                                                (value.products_count ?
                                                                                    <li key={value.id} className={`${(value.children_recursive.length > 0) ? 'has-children' : ''} ${((openToggle === value.id)) ? 'active' : ''}`}>
                                                                                        <div className="list-text">
                                                                                            <NavLink to={`/product-category/${cat.slug}/${child.slug}/${value.slug}`} className={""}>
                                                                                                {value.name}
                        ...
                        <div className="col-md-7">
                            ...
                            <div className="widget">
                                <h4><NavLink to="/">Home</NavLink> / <NavLink to={`/brand/${brand}`}>{brand}</NavLink>
